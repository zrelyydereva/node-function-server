...
module.exports = function(argv) {
    ...
    return function srvMain(request, response, next) {
        ...
        var paths = (request.path).substring(_argv.mount.length).split("/");
        ...
        if (tmpStat != null) {
            ...
            for (var k in request.query) {
                if (k == "token") continue;
                if (request.query.hasOwnProperty(k)) args.push((request.query[k] != "" ? request.query[k] : k));
            }
            for (var k in request.body) {
                if (request.body.hasOwnProperty(k)) args.push((request.body[k] != "" ? request.body[k] : k));
            }
            //ユーザ認証があれば
            ...
            try {
                ...
                //関数を実行
                var ret = _scripts[nPath].context[command].apply(_scripts[nPath].context[command], args, user);
                if (ret && ret.then) {
